...
import { OpenAICompatEngine } from './openai-compat/index.js';
...
export function createEngine(config: BotConfigBase, logger: Logger, override?: EngineName): Engine {
  ...
  switch (name) {
    ...
    case 'openai-compat':
      return new OpenAICompatEngine(config, logger);
    default: {
...
export { OpenAICompatEngine } from './openai-compat/index.js';
...
export { ClaudeExecutor, StreamProcessor, SessionManager, extractImagePaths } from './claude/index.js';
export type {
  UserSession,
  SDKMessage,
  ExecutionHandle,
