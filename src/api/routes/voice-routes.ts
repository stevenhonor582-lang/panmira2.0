...
import { minimaxSpeechGenerate, resolveMiniMaxApiKey } from '../minimax-generate.js';
  ...
  if (method === 'POST' && (url === '/api/tts' || url.startsWith('/api/tts?'))) {
    ...
    if (!provider) {
      jsonResponse(res, 400, {
        error:
          'No TTS provider available. Configure VOLCENGINE_TTS_APPID + VOLCENGINE_TTS_ACCESS_KEY, or OPENAI_API_KEY, or ELEVENLABS_API_KEY.',
      });
      return true;
    }
    ...
    const ttsText = text.length > maxChars ? text.slice(0, maxChars - 10) + '... (truncated)' : text;

    let audioBuffer: Buffer;
    if (provider === 'elevenlabs') {
      audioBuffer = await elevenlabsTTS(ttsText, voice);
    } else if (provider === 'minimax') {
      if (!ctx.providerConfigStore) {
        jsonResponse(res, 400, { error: 'MiniMax TTS requires provider config store' });
        return true;
      }
      const apiKey = await resolveMiniMaxApiKey(ctx.providerConfigStore);
      audioBuffer = await minimaxSpeechGenerate(apiKey, ttsText, { voiceId: voice || 'male-qn-qingse' });
