...
export async function handleTemplateRoutes(
  ...
  if (method === 'GET' && url === '/api/templates') {
    const result = await pool.query(
      'SELECT id, name, display_name, description, category, template_type, version, is_active, default_skills, default_agents, default_knowledge_folders, default_engine, default_model, created_at FROM templates ORDER BY category, name'
    );
  ...
  if (method === 'POST' && url === '/api/templates') {
    ...
    const result = await pool.query(
      `INSERT INTO templates (name, display_name, description, category, template_type, system_prompt, default_skills, default_agents, default_knowledge_folders, default_engine, default_model)
       ...
       ON CONFLICT (name) DO UPDATE SET
         ...
         default_engine=$10, default_model=$11, updated_at=now()
       RETURNING *`,
  ...
  if (method === 'PUT' && putMatch) {
    ...
    for (const [key, value] of Object.entries(body)) {
      ...
      paramCount++;
      if (['default_skills', 'default_agents', 'default_knowledge_folders'].includes(key)) {
