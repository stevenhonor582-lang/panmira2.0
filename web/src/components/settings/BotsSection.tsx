    ...
    <>
      ...
      >
        ...
        <div className={styles.row}>
          ...
          <label className={styles.field}>
            <span className={styles.label}>{t('bots.remarkLabel') || '备注'}</span>
            <input className={styles.input} value={botRemark} onChange={(e) => setBotRemark(e.target.value)} placeholder="可选, 如: 内容创作" autoComplete="off" />
            <div className={styles.formHint}>显示为: {botName}{botRemark ? '--' + botRemark : ''}</div>
          </label>
        </div>
        <div className={styles.row}>
          <label className={styles.field}>
