...
import {
  Search, Loader2, AlertTriangle, RefreshCw, FileSearch, Database, ListTree, X, Filter,
} from "lucide-react";
...
import type { AlertItem, BotGroup, WeeklyBucket } from "./_components/types";
import { AlertsGrouped, bucketByWeek } from "./_components/alerts-grouped";
import { SeverityBarChart } from "./_components/severity-bar-chart";
...
const TYPE_BADGE: Record<string, { tone: "default" | "destructive" | "secondary"; label: string }> = {
  ...
  warning: { tone: "secondary", label: "warning" },
    ...
    <div className="space-y-5">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          告警监控(按 bot/agent 聚合) + 异常诊断排查 — 一个入口查全部
        </p>
      </header>
      ...
      <Tabs defaultValue="alerts" className="space-y-5">
        <TabsList>
          <TabsTrigger value="alerts" className="gap-1.5">
            <AlertTriangle className="size-3.5" />告警监控
          </TabsTrigger>
          <TabsTrigger value="diagnose" className="gap-1.5">
            <FileSearch className="size-3.5" />异常诊断
          </TabsTrigger>
        </TabsList>
...
function AlertsTab() {
  ...
  const bots = useMemo(
    () => Array.from(new Set((alerts ?? []).map((a) => a.bot_name))).sort(),
    [alerts],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return (alerts ?? []).filter((a) => {
      ...
      if (q) {
        const hay = `${a.bot_name} ${a.error_message ?? ""} ${a.type}`.toLowerCase();
        if (!hay.includes(q)) return false;
      }
      return true;
    });
  }, [alerts, botFilter, typeFilter, severityFilter, search]);

  const weeklyBuckets: WeeklyBucket[] = useMemo(() => bucketByWeek(filtered), [filtered]);

  const reset = () => {
    setBotFilter("__all__");
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          task_failed + error 事件流 · 共 {(alerts ?? []).length} 条 · 显示 {filtered.length} 条
        </p>
      ...
      <SeverityBarChart buckets={weeklyBuckets} />

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <Filter className="size-3.5 text-muted-foreground" />筛选
          </CardTitle>
          <CardDescription>按 bot / 类型 / 严重度 / 关键字</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap items-end gap-3">
          <div className="space-y-1.5 min-w-[140px] flex-1">
            ...
            <div className="relative">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="关键字 / bot / 错误消息" className="pl-8" />
            </div>
          </div>
      ...
      {loading ? (
        ...
        <AlertsGrouped alerts={filtered} />
      )}
    </div>
  );
}

function inferSeverity(a: AlertItem): BotGroup["severity"] {
  if (a.severity) return a.severity;
  if (a.type === "task_failed") return "error";
  if (a.type === "error") return "error";
  if (a.type === "warning") return "warning";
  return "info";
}

function DiagnoseTab() {
  const [taskId, setTaskId] = useState("");
  const [result, setResult] = useState<null | { found: number; session?: Record<string, unknown>; events?: Array<Record<string, unknown>> }>(null);
  ...
  const search = async () => {
    ...
    try {
      const r = await api<{ found: number; session?: Record<string, unknown>; events?: Array<Record<string, unknown>> }>(
      ...
      <Card>
        ...
        <CardContent className="flex gap-2">
          ...
          <Button onClick={search} disabled={loading || !taskId.trim()} className="gap-1.5">
            {loading ? <Loader2 className="size-3.5 animate-spin" /> : <Search className="size-3.5" />}诊断
          </Button>
        </CardContent>
      </Card>
        ...
        <>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <Database className="size-4 text-muted-foreground" />Session 详情
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          {result.events && result.events.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <ListTree className="size-4 text-muted-foreground" />事件时间线 ({result.events.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {result.events.map((e, i) => (
                    <div key={i} className="rounded-md border border-border bg-card p-3 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge variant={e.error_message ? "destructive" : "secondary"}>{String(e.type ?? "—")}</Badge>
                        <span className="text-sm font-medium">{String(e.bot_name ?? "—")}</span>
                        <span className="text-[11px] text-muted-foreground tabular-nums">{fmtTs(e.timestamp as string | number | undefined)}</span>
                      </div>
                      {typeof e.prompt === "string" && (
                        <p className="text-xs text-muted-foreground line-clamp-2">
                          <span className="text-[10px] uppercase tracking-wider mr-1">prompt</span>{e.prompt}
                        </p>
                      )}
                      {typeof e.response_preview === "string" && (
                        <p className="text-xs line-clamp-3">
                          <span className="text-[10px] uppercase tracking-wider text-muted-foreground mr-1">response</span>{e.response_preview}
                        </p>
                      )}
                      {typeof e.error_message === "string" && (
                        <p className="text-xs text-destructive font-mono break-all">{e.error_message}</p>
                      )}
                    </div>
                  ))}
                </div>
