...
if [[ "$OS" != "Linux" && "$OS" != "Darwin" ]]; then
  error "Unsupported OS: $OS. Panmira supports Linux and macOS."
  exit 1
fi
...
if [[ "$SKIP_CONFIG" == "false" ]]; then
  # Generate .env
  {
    echo "# Panmira Configuration (generated by install.sh)"
...
success "MetaMemory will start automatically with Panmira on port 8100"
...
# Install mb() shell shortcut for Panmira API (Agent Bus, Scheduling, Bot Management)
