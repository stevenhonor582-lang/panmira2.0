...
### Added
- `install.sh`: `--dir <path>` / `-d <path>` flag to customize the install directory (priority: `--dir` > `PANMIRA_HOME` env > interactive prompt > `~/panmira`). Non-default paths are persisted to `~/.bashrc` / `~/.zshrc` so the `mb`/`mm`/`panmira` CLIs find the install in new shells.
- `install.ps1`: matching `-Dir <path>` parameter on Windows; non-default paths persisted via user-level `PANMIRA_HOME` environment variable.
